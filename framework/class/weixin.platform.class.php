...
class WeiXinPlatform extends WeiXinAccount {
	...
	public function build($appid){
		...
		$post['ext_json']=array('extAppid'=>$wxapp['key'],'ext'=>json_decode($ext,true));
		load()->func('communication');
		$result=ihttp_request($url,$post);
		$result=$result['content'];
		$result=json_decode($result,true);
		if($result['errcode']){
			itoast('上传小程序代码出错：'.$result['errmsg'], url('account/manage'), 'error');
		}
		//提交审核
		$url='https://api.weixin.qq.com/wxa/submit_audit?access_token='.$wxapp['access_token'];
		$post=array('item_list'=>json_decode($module['item_list'],true));
		$result=ihttp_request($url,$post);
		$result=$result['content'];
		$result=json_decode($result,true);
		if($result['errcode']){
			itoast('提交审核小程序代码出错：'.$result['errmsg'], url('account/manage'), 'error');
		}
