...
if ($do == 'front_download') {
	...
	$wxapp=pdo_getcolumn('account_wxapp',array('uniacid'=>$_W['uniacid']),array('status','fail_reason'));
	//1上传待审核，2审核成功，3审核失败
	if($wxapp['status']==0){
		$show['sc']='未上传';
		$show['sh']='未审核';
	...
	if($wxapp['fail_reason']){
		$show['reason']=$wxapp['fail_reason'];
	}
